      ...
      if (measures.dataEntries.length) {
        measures.dataEntries.forEach(entry => {
          if (isFontResource(entry) && (entry.response.content.size > 0)) {
            ...
            if (url_toshow.length > 80) {
              url_toshow = url_toshow.substring(0, 80) + "...";
            }
            this.detailComment += url_toshow + " " + Math.round(entry.response.content.size / 1000) + "KB <br>";
          }
        });
      }
      if (this.specificMeasures.totalFontsSize > 10000) {
        this.complianceLevel = 'C';
      }
      else if (this.specificMeasures.totalFontsSize > 0) {
        this.complianceLevel = 'B';
      }
      if (this.specificMeasures.totalFontsSize > 0) {
        this.comment = chrome.i18n.getMessage("rule_UseStandardTypefaces_Comment", String(Math.round(this.specificMeasures.totalFontsSize / 1000)));
      }
    },
